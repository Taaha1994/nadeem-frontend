  ...
  return (
    <div style={{ marginBottom:22 }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${C.navyDark},${C.blueDeep}44)`, border:`2px solid ${riskColor}66`, borderRadius:14, padding:"22px 24px", marginBottom:12 }}>
        <div style={{ fontSize:10, letterSpacing:"0.2em", textTransform:"uppercase", color:C.gold, fontWeight:700, marginBottom:16 }}>
          ⚡ Unified Signing Risk Score
        </div>
        <div style={{ display:"flex", alignItems:"flex-start", gap:20, flexWrap:"wrap", marginBottom:16 }}>
          {/* Big number */}
          <div style={{ textAlign:"center" }}>
            <div style={{ fontSize:10, color:C.mutedDark, marginBottom:4, letterSpacing:"0.1em", textTransform:"uppercase" }}>Combined Risk</div>
            <div style={{ position:"relative", width:90, height:90, margin:"0 auto" }}>
              <svg viewBox="0 0 90 90" style={{ transform:"rotate(-90deg)" }}>
                <circle cx="45" cy="45" r="38" fill="none" stroke={C.navy} strokeWidth="8"/>
                <circle cx="45" cy="45" r="38" fill="none" stroke={riskColor} strokeWidth="8"
                  strokeDasharray={`${2 * Math.PI * 38 * combinedRisk / 100} ${2 * Math.PI * 38}`}
                  strokeLinecap="round"/>
              </svg>
              <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", textAlign:"center" }}>
                <div style={{ fontSize:22, fontWeight:800, color:riskColor, lineHeight:1 }}>{combinedRisk}</div>
                <div style={{ fontSize:9, color:C.mutedDark }}>/100</div>
              </div>
            </div>
            <div style={{ marginTop:6, display:"inline-block", background:riskColor+"20", border:`1px solid ${riskColor}50`, borderRadius:6, padding:"3px 10px", fontSize:11, fontWeight:700, color:riskColor }}>{riskBand}</div>
          </div>

          {/* Components */}
          <div style={{ flex:1, minWidth:180 }}>
            <div style={{ fontSize:11, color:C.mutedDark, marginBottom:10 }}>Score breakdown ({weights.nadeem}% clinical · {weights.availability}% historical)</div>
            {/* Nadeem component */}
            <div style={{ marginBottom:10 }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, marginBottom:4 }}>
                <span style={{ color:C.muted }}>Nadeem Score (clinical risk)</span>
                <span style={{ color:C.blue, fontWeight:700 }}>{nadeemScore}/13 → {nadeemRisk}% risk</span>
              </div>
              <div style={{ height:6, background:C.navy, borderRadius:3, overflow:"hidden" }}>
                <div style={{ height:"100%", width:`${nadeemRisk}%`, background:C.blue, borderRadius:3 }}/>
              </div>
            </div>
            {/* Availability component */}
            <div style={{ marginBottom:10 }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, marginBottom:4 }}>
                <span style={{ color:C.muted }}>Availability Index (historical)</span>
                <span style={{ color:availIndex?.color || C.gold, fontWeight:700 }}>{availIndex?.score}/10 → {availRisk}% risk</span>
              </div>
              <div style={{ height:6, background:C.navy, borderRadius:3, overflow:"hidden" }}>
                <div style={{ height:"100%", width:`${availRisk}%`, background:availIndex?.color || C.gold, borderRadius:3 }}/>
              </div>
            </div>
            {/* Recommendation */}
      ...
      <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:12 }}>
        ...
        {availIndex && (
          <div style={{ flex:1, minWidth:200, background:C.navyDark, border:`1px solid ${availIndex.color}33`, borderRadius:10, padding:"16px 18px" }}>
            <div style={{ fontSize:10, letterSpacing:"0.15em", textTransform:"uppercase", color:C.mutedDark, fontWeight:700, marginBottom:10 }}>📊 Availability Index</div>
