...
// AVAILABILITY GAUGE
// ══════════════════════════════════════════════════════════════════════════
function AvailabilityGauge({ availability, seasonStats }) {
  const { pctLow, pctMid, pctHigh, gamesLow, gamesMid, gamesHigh, confidence, riskLabel, riskColor, components } = availability;

  return (
    <div style={{ background:`linear-gradient(135deg,${C.navyDark},${C.blueDeep}44)`, border:`1px solid ${riskColor}44`, borderLeft:`4px solid ${riskColor}`, borderRadius:12, padding:"22px 24px", marginBottom:22 }}>
      <div style={{ fontSize:10, letterSpacing:"0.2em", textTransform:"uppercase", color:C.gold, fontWeight:700, marginBottom:14 }}>
        🎯 Predicted Availability — Next Season
      </div>

      {/* Main prediction */}
      <div style={{ display:"flex", gap:16, alignItems:"flex-end", marginBottom:18, flexWrap:"wrap" }}>
        <div>
          <div style={{ fontSize:11, color:C.mutedDark, marginBottom:4 }}>Predicted games available</div>
          <div style={{ display:"flex", alignItems:"baseline", gap:8 }}>
            <span style={{ fontSize:56, fontWeight:800, color:riskColor, lineHeight:1 }}>{gamesMid}</span>
            <span style={{ color:C.mutedDark, fontSize:16 }}>/ 46</span>
          </div>
          <div style={{ fontSize:12, color:C.muted, marginTop:3 }}>Range: {gamesLow}–{gamesHigh} games ({pctLow}–{pctHigh}%)</div>
        </div>
        <div style={{ flex:1, textAlign:"right" }}>
          <div style={{ display:"inline-block", background:riskColor+"20", border:`1px solid ${riskColor}50`, borderRadius:8, padding:"8px 16px", marginBottom:8 }}>
            <div style={{ fontSize:20, fontWeight:800, color:riskColor }}>{pctMid}%</div>
            <div style={{ fontSize:11, color:riskColor, fontWeight:600 }}>availability</div>
          </div>
          <div style={{ fontSize:12, fontWeight:700, color:riskColor }}>{riskLabel}</div>
          <div style={{ fontSize:11, color:C.mutedDark, marginTop:2 }}>Confidence: {confidence}</div>
        </div>
      </div>

      {/* Range bar */}
      <div style={{ marginBottom:16 }}>
        <div style={{ position:"relative", height:10, background:C.navy, borderRadius:5, overflow:"visible" }}>
          {/* Range band */}
          <div style={{ position:"absolute", left:`${pctLow}%`, width:`${pctHigh-pctLow}%`, height:"100%", background:riskColor+"44", borderRadius:5 }}/>
          {/* Central point */}
          <div style={{ position:"absolute", left:`${pctMid}%`, transform:"translateX(-50%)", top:-3, width:16, height:16, background:riskColor, borderRadius:"50%", border:`2px solid ${C.navyDeep}` }}/>
        </div>
        <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:C.mutedDark, marginTop:6 }}>
          <span>0%</span><span>25%</span><span>50%</span><span>75%</span><span>100%</span>
        </div>
      </div>

      {/* Algorithm components */}
      <div style={{ borderTop:`1px solid ${C.navy}`, paddingTop:12 }}>
        <div style={{ fontSize:10, letterSpacing:"0.15em", textTransform:"uppercase", color:C.mutedDark, fontWeight:700, marginBottom:10 }}>Algorithm Components</div>
        <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
          {[
            ["Nadeem Score base", components.scoreBase + "%"],
            ["3-season trend",    (components.trendAdj >= 0 ? "+" : "") + components.trendAdj + "%"],
            ["Age curve",         (components.ageAdj >= 0 ? "+" : "") + components.ageAdj + "%"],
            ["Injury recency",    (components.injuryRecencyAdj >= 0 ? "+" : "") + components.injuryRecencyAdj + "%"],
            ["Historical anchor", (components.historicalAdj >= 0 ? "+" : "") + components.historicalAdj + "%"],
          ].map(([label, val]) => (
            <div key={label} style={{ background:C.navyDeep, border:`1px solid ${C.navy}`, borderRadius:6, padding:"5px 10px", fontSize:11 }}>
              <span style={{ color:C.mutedDark }}>{label}: </span>
              <span style={{ color:parseFloat(val) >= 0 ? "#86efac" : "#f87171", fontWeight:700 }}>{val}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {seasonStats && seasonStats.length > 0 && (
        <div style={{ marginTop:14, borderTop:`1px solid ${C.navy}`, paddingTop:12 }}>
          <div style={{ fontSize:10, letterSpacing:"0.15em", textTransform:"uppercase", color:C.mutedDark, fontWeight:700, marginBottom:10 }}>Availability History</div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {seasonStats.slice(0,5).map(s => {
              const pct = Math.min(100, Math.round((s.apps / 46) * 100));
              const col = pct >= 70 ? "#4ade80" : pct >= 45 ? C.gold : "#f87171";
              return (
                <div key={s.season} style={{ flex:1, minWidth:70, background:C.navyDeep, border:`1px solid ${C.navy}`, borderRadius:8, padding:"8px 10px", textAlign:"center" }}>
                  <div style={{ fontSize:10, color:C.mutedDark, marginBottom:4 }}>{s.season}</div>
                  <div style={{ fontSize:16, fontWeight:800, color:col }}>{s.apps}</div>
                  <div style={{ fontSize:10, color:C.mutedDark }}>apps</div>
                  <div style={{ marginTop:4, height:3, background:C.navy, borderRadius:2, overflow:"hidden" }}>
...
function AutoSigningScore({ playerData, onClear }) {
  const { flags: autoFlags, nadeemScore: autoScore, availability, seasonStats, injuries, dataCompleteness, name, age, position, club, nationality } = playerData;
    ...
    <div>
      ...
      <div style={{ background:"#fff", color:"#111", padding:"32px 40px", fontFamily:"Arial,sans-serif", borderRadius:8 }}>
        ...
        <div style={{ display:"flex", gap:16, marginBottom:20 }}>
          ...
          {availability && (
            <div style={{ flex:1, textAlign:"center", border:`2px solid ${availability.riskColor}`, background:availability.riskColor+"10", borderRadius:10, padding:"16px" }}>
              <div style={{ fontSize:11, color:"#888", textTransform:"uppercase", marginBottom:4 }}>Predicted Availability</div>
              <div style={{ fontSize:32, fontWeight:800, color:availability.riskColor, lineHeight:1 }}>{availability.pctMid}%</div>
              <div style={{ fontSize:12, color:"#888" }}>{availability.gamesLow}–{availability.gamesHigh} games</div>
            </div>
          )}
        </div>
        {/* Recommendation */}
  ...
  return (
    <div>
      ...
      {availability && <AvailabilityGauge availability={availability} seasonStats={seasonStats} />}
